    ...
    for item in RomMap:
        if old_address != item.address:
            WriteSectionInFile(
                file,
                filedir,
                "Unknown_" + hex(old_address) + ".bin",
                old_address,
                item.address - old_address,
            )
            output += (
                hex(old_address)
                + " "
...
def diagnose(filename, filedir, outFileList, debug, handle_progress):
    ...
    handle_progress(46, "Loading fst.bin...")
    ...
    handle_progress(100, "Outputting _header...")
...
def RomFSdiagnose(file, base_address, filedir="", debug=False):
    ...
    for i in range(num_entries):
        ...
        if ReadByte(file, base_address + file_offset) == 1:
            dirs = (
                RomFS_Dirdiagnose(
                    file, base_address, file_offset, num_entries * 0xC, "", debug
                )
                + "/"
            )
        elif ReadByte(file, base_address + file_offset) == 0:
            output += RomFS_Filediagnose(
                file,
                base_address,
                file_offset,
                num_entries * 0xC,
...
def RomFS_Dirdiagnose(file, base_address, offset, string_table, path, debug=False):
    ...
    offset_string = (
        (ReadWord(file, base_address + offset + 0x0)) & 0xFFFFFF
    ) + string_table
...
def RomFS_Filediagnose(
    file, base_address, offset, string_table, path, filedir="", debug=False
):
    output = ""
...
def DolFSdiagnose(file, base_address, filedir="", debug=False):
    ...
    for i in range(6):
        WriteSectionInFile(
            file,
            filedir + "code/",
            "Text_" + hex(TextMem[i]) + ".bin",
            base_address + TextPos[i],
            TextSize[i],
        )
    ...
    for i in range(10):
        WriteSectionInFile(
            file,
            filedir + "code/",
            "Data_" + hex(DataMem[i]) + ".bin",
            base_address + DataPos[i],
            DataSize[i],
        )
...
class fstDir:
    ...
    def addSubDir(self, name):
        ...
        self.subDirs = sorted(
            self.subDirs, key=lambda fstDir: fstDir.lower.replace("_", "}")
        )
    ...
    def createFST(self, string_table_length, parentID, ownID):
        if "Unknown" in self.name:
            print(f"{self.name} {self.FileID} {self.FileID == -0x1}")
        if self.FileID == -0x1:
            return ([], "")
...
def print_dir_tree(fst_dir, iteration=0):
    cur_dir = fst_dir
    sub_dirs = sorted(cur_dir.subDirs, key=lambda x: x.flag, reverse=True)

    indent = "    " * iteration
    for item in sub_dirs:
        if item.flag == 0:  # If the FstDir is a file
            print(
                f"{indent}~ {item.name} ({hex(item.FileSize)}) Offset: {hex(item.offset)}"
            )
        if item.flag == 1:  # If the FstDir is a directory
            print(f"{indent}> {item.name}")
            print_dir_tree(item, iteration + 1)  # Print the directory
...
#         nargs=4,
#         type=str,
#         metavar=("inputDir", "fstFile", "fstMap", "outputFile"),
#         help="pack files into a gamecube iso",
#     )
#     group.add_argument(
#         "-unpack",
#         action="store",
#         nargs=3,
#         type=str,
#         metavar=("inputFile", "outputDir", "outFilelist"),
#         help="unpack files from a gamecube iso",
#     )
...
#     if args.pack:
#         output = create_rom(
#             args.pack[0], args.pack[3], args.pack[1], args.pack[2], debug
#         )
#     if args.unpack:
