...
import time
import hashlib
import tools.gladiushashes as gladiushashes
import queue
import threading
...
def GetNumberedFilenameOfFile(file, Offset, i):
    ...
    if ReadHWord(file, Offset) == 0x2F2F:
        ...
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x0D0A0D0A) and (
        ReadHWord(file, Offset + 4) == 0x2F2F
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x436F7079) and (
        ReadWord(file, Offset + 4) == 0x72696768
    ):
        filename = str(i) + ".txt"
        ...
        filename = str(i) + ".pak1"
    elif (ReadWord(file, Offset) == 0x23233832) and (
        ReadWord(file, Offset + 4) == 0x32300D0A
    ):
        filename = str(i) + ".txt"
        ...
        filename = str(i) + ".pms2"
    elif (ReadWord(file, Offset) == 0x40656368) and (
        ReadWord(file, Offset + 4) == 0x6F206F66
    ):
        filename = str(i) + ".bat"
        ...
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x4C4F4341) and (
        ReadWord(file, Offset + 4) == 0x544F5253
    ):
        filename = str(i) + ".locators"
    elif (ReadWord(file, Offset) == 0x312C2242) and (
        ReadWord(file, Offset + 4) == 0x6174746C
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x66756E63) and (
        ReadWord(file, Offset + 4) == 0x74696F6E
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x52454D20) and (
        ReadWord(file, Offset + 4) == 0x2D2D2047
    ):
        filename = str(i) + ".bat"
    elif (ReadWord(file, Offset) == 0x53554254) and (
        ReadWord(file, Offset + 4) == 0x49544C45
    ):
        ...
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x4E554D45) and (
        ReadWord(file, Offset + 4) == 0x44474553
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x4E554D43) and (
        ReadWord(file, Offset + 4) == 0x52454449
    ):
        filename = str(i) + ".txt"
        ...
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x54494E54) and (
        ReadWord(file, Offset + 4) == 0x494E473A
    ):
        filename = str(i) + ".tinting.txt"
    elif ReadWord(file, Offset) == 0x0D0A2066:
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x4D4F4449) and (
        ReadWord(file, Offset + 4) == 0x54454D53
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x5343454E) and (
        ReadWord(file, Offset + 4) == 0x453A0909
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x496E7465) and (
        ReadWord(file, Offset + 4) == 0x72666163
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x4C656167) and (
        ReadWord(file, Offset + 4) == 0x75655374
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x5C70726F) and (
        ReadWord(file, Offset + 4) == 0x6A656374
    ):
        filename = str(i) + ".txt"
        ...
        filename = str(i) + ".bat"
    elif (ReadWord(file, Offset) == 0x6275696C) and (
        ReadWord(file, Offset + 4) == 0x6470616B
    ):
        filename = str(i) + ".txt"
    elif (ReadWord(file, Offset) == 0x5363686F) and (
        ReadWord(file, Offset + 4) == 0x6F6C5374
    ):
        filename = str(i) + ".txt"
...
def diagnose(filename, filedir, outFileList, debug, handle_progress):
    handle_progress(0, "Extracting bec...")
    file = open(filename, "rb+")
    header_output = diagnose2(file, filedir, debug, handle_progress)
...
def diagnose2(file, filedir, debug, handle_progress):
    output = ""
    ...
    output += (
        hex(header.FileAlignment)
        + " "
        + hex(header.NrOfFiles)
        + " "
        + hex(header.HeaderMagic)
        + "\n"
    )
    ...
    for i in range(header.NrOfFiles):
        ...
        percent = ((i + 1) / int(header.NrOfFiles)) * 100
        handle_progress(percent, f"Extracting {filename}...")
        ...
        if OffsetCorrection[i] > 0:
            ...
            WriteSectionInFile(
                fByteArray, filedir, filename2, DataOffset[i], OffsetCorrection[i]
            )
        ...
        output += (
            ...
            + filename2
            + '" '
            + hex(PathHash[i]).rstrip("L")
            + " "
            + hex(DataOffset[i]).rstrip("L")
            + " "
            + hex(OffsetCorrection[i]).rstrip("L")
            + " "
            + hex(Data3[i]).rstrip("L")
            + " "
            + hex(DataSize[i]).rstrip("L")
            + " "
            + hex(CheckSums1[i]).rstrip("L")
            + " "
            + hex(CheckSums2[i]).rstrip("L")
            + "\n"
        )
...
class RomSection:
    def __init__(
        self, name, name2, hash, address, size2, flags, size, checksum, checksum2
    ):
        self.name = name
        self.name2 = name2
        self.hash = hash
...
def addRomSection(
    name, name2, hash, address, size2, flags, size, checksum, checksum2
):  # filename, hash?, offset, offsetcorrection, flags?, filesize, checksum?
    RomMap.append(
        RomSection(name, name2, hash, address, size2, flags, size, checksum, checksum2)
    )
...
def createBecArchive(dir, filename, becmap, gc, debug=False):
    ...
    with open(becmap) as fin:
        for line in fin:
            ...
            if len(words) == 3:
                ...
                words = (
                    [words_temp[1]] + [words_temp[3]] + words_temp[4].split()
                )  # ?, offset, flags?, filesize
                # print(words)
                if len(words) == 9:
                    ...
                    addRomSection(
                        ...
                        int(words[7], 16),
                        int(words[8], 16),
                    )  # filename, filename2, hash?, offset, filesize2, flags?, filesize, checksum?, checksum2?
    ...
    for i in range(len(RomMap)):
        if RomMap[i].flags != 0:
            for j in range(len(RomMap)):
                if (
                    (i != j)
                    and (RomMap[j].flags == 0)
                    and (RomMap[i].address == RomMap[j].address)
                ):
    ...
    for item in RomMap:
        ...
        output_rom.write(
            struct.pack(
                "<BBB",
                int((item.size2 >> 0) & 0xFF),
                int((item.size2 >> 8) & 0xFF),
                int((item.size2 >> 16) & 0xFF),
            )
        )
    ...
    for item in RomMap:
        ...
        if item.size2 > 0:
            ...
            output_rom.write(
                struct.pack("<II", int(item.checksum), int(item.checksum2))
            )  # checksum? PS2Exclusive
            alignFileSizeWithZeros(output_rom, output_rom.tell(), filealignment)
        ...
        if ((item.size2 == 0) and (item.size > 0)) or (gc == 1):
            output_rom.write(
                struct.pack("<II", item.checksum, item.checksum2)
            )  # checksum? GCExclusive
        ...
        if (item.size2 == 0) and (item.size == 0) and (gc == 0):
            output_rom.write(
                b"\0" * filealignment
            )  # write dvd filler material into archive PS2Exclusive
